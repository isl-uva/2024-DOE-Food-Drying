...
class functions():
    ...
    def obs_dict_to_array(obs_dict):
        ...
        actor_state = np.vstack((agent_0_state, agent_1_state, agent_2_state), dtype=np.float32)
        critic_state = np.concatenate((agent_0_state, agent_1_state, agent_2_state), axis=0, dtype=np.float32)
