...
from Dymola_Env_Bypass import DymolaEnv, scale_and_clamp_values
import random
import json
...
for epoch in range(1):
    ...
    for i in range(random_step_size):
        action_output = [0.4, env.action_space[0].sample().item(), env.action_space[1].sample().item(), 0.4, 273.15 + 46,
                  env.action_space[2].sample().item(), env.action_space[3].sample().item() ]
...
for epoch in range(num_epoch):
    try:
        ...
        for i in range(random_step_size):
            action_output = [0.4, env.action_space[0].sample().item(), env.action_space[1].sample().item(), 0.2, 273.15 + 46,
                      env.action_space[2].sample().item(), env.action_space[3].sample().item()]
